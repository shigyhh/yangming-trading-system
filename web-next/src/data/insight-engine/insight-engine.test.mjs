...
test("all scene mirror ids are explicitly mapped before ritual reveal", () => {
  const officialMirrorIds = new Set(["chase", "hold", "fantasy", "gamble", "herd", "hesitate", "delay", "anxiety", "liangzhi"])

  for (const { scene } of manifestScenePairs) {
    const mirrorId = scene.mirrorId
    const hasOfficialMirror = officialMirrorIds.has(mirrorId)
    const hasRitualAlias = ritualFlowSource.includes(`\n  ${mirrorId}: "`)

    assert.equal(hasOfficialMirror || hasRitualAlias, true, `${scene.sceneId} mirrorId ${mirrorId} must not fall back to chase`)
  }
})
...
test("official scene 01 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene01.items[0].tradeMoment, "盘中突然拉出长阳时")
  assert.equal(scene01.items[0].os, "我次奥，真拉了。")
  assert.equal(scene01.items[0].hiddenThought, "我又慢了。")
  assert.equal(scene01.items[0].reflection, "这一声我次奥，不是惊讶，是后悔。")
  assert.equal(scene01.items[9].tradeMoment, "看着大阳线越拉越远时")
...
test("official scene 02 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene02.items[0].tradeMoment, "看着它连涨好几天时")
  assert.equal(scene02.items[0].os, "我次奥，又涨了。")
  assert.equal(scene02.items[0].hiddenThought, "我又错过了。")
  assert.equal(scene02.items[0].reflection, "这一声我次奥，不是惊讶，是后悔。")
...
test("official scene 03 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene03.items[0].tradeMoment, "账户刚开始浮盈时")
  ...
  assert.equal(scene03.items[9].tradeMoment, "收盘后回看整段走势时")
...
test("official scene 04 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene04.items[0].tradeMoment, "刚卖完它又往上拉时")
  assert.equal(scene04.items[0].os, "我次奥，还涨。")
  assert.equal(scene04.items[0].hiddenThought, "我又卖早了。")
  assert.equal(scene04.items[0].reflection, "这一声我次奥，不是惊讶，是后悔。")
  assert.equal(scene04.items[9].tradeMoment, "关软件前还在回想时")
...
test("official scene 05 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene05.items[0].tradeMoment, "浮盈开始往回吐时")
  ...
  assert.equal(scene05.items[9].tradeMoment, "关软件前反复懊悔时")
...
test("official scene 06 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene06.items[0].tradeMoment, "浮盈刚开始回吐时")
  ...
  assert.equal(scene06.items[9].tradeMoment, "卖出后盯着账户时")
...
test("official scene 07 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene07.items[0].tradeMoment, "价格刚触到止损线时")
  ...
  assert.equal(scene07.items[9].tradeMoment, "关软件前还没卖时")
...
test("official scene 08 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene08.items[0].tradeMoment, "亏损已经压得很重时")
  ...
  assert.equal(scene08.items[9].tradeMoment, "关软件前选择放着时")
...
test("official scene 09 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene09.items[0].tradeMoment, "账户继续往下亏时")
  ...
  assert.equal(scene09.items[9].tradeMoment, "关软件前还想着回本时")
...
test("official scene 10 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene10.items[0].tradeMoment, "补完仓又跌破低点时")
  ...
  assert.equal(scene10.items[9].tradeMoment, "关软件前还盼着回来时")
...
test("official scene 11 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene11.items[0].tradeMoment, "连亏几笔后盯着账户时")
  ...
  assert.equal(scene11.items[9].tradeMoment, "收盘前还想扳回来时")
...
test("official scene 12 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene12.items[0].tradeMoment, "连赚几笔后手感发热时")
  ...
  assert.equal(scene12.items[9].tradeMoment, "收盘后回看全红记录时")
...
test("official scene 13 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene13.items[0].tradeMoment, "看到一个自认好机会时")
  ...
  assert.equal(scene13.items[9].tradeMoment, "关软件前想着干票大的时")
...
test("official scene 14 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene14.items[0].tradeMoment, "看到自认确定机会时")
  ...
  assert.equal(scene14.items[9].tradeMoment, "收盘前想靠这一把翻身时")
...
test("official scene 15 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene15.items[0].tradeMoment, "空仓看着行情上涨时")
  ...
  assert.equal(scene15.items[9].tradeMoment, "关软件前暗暗发誓时")
...
test("official scene 16 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene16.items[0].tradeMoment, "开盘走势偏离预案时")
  ...
  assert.equal(scene16.items[9].tradeMoment, "复盘时盯着改坏的那笔时")
...
test("official scene 17 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene17.items[0].tradeMoment, "刚止损它就反弹时")
  ...
  assert.equal(scene17.items[9].tradeMoment, "收盘后盯着那笔止损时")
...
test("official scene 18 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene18.items[0].tradeMoment, "刚止盈它又冲高时")
  assert.equal(scene18.items[0].os, "我次奥，还涨。")
  assert.equal(scene18.items[0].hiddenThought, "我又卖早了。")
  assert.equal(scene18.items[0].reflection, "最难受的不是卖出，是卖完以后它还在涨。")
  assert.equal(scene18.items[9].tradeMoment, "关软件前还在想卖点时")
...
test("official scene 19 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene19.items[0].tradeMoment, "开盘刚跳出异动时")
  assert.equal(scene19.items[0].os, "我次奥，动了。")
  assert.equal(scene19.items[0].hiddenThought, "我不能错过。")
  assert.equal(scene19.items[0].reflection, "开盘最危险的不是波动，是冲动。")
  assert.equal(scene19.items[9].tradeMoment, "收盘复盘看到开盘那笔时")
...
test("official scene 20 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene20.items[0].tradeMoment, "临近收盘还空着手时")
  ...
  assert.equal(scene20.items[9].tradeMoment, "收盘后看着尾盘那笔时")
...
test("official scene 21 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene21.items[0].tradeMoment, "收盘后打开账户时")
  ...
  assert.equal(scene21.items[9].tradeMoment, "睡前还在回想交易时")
...
test("official scene 22 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene22.items[0].tradeMoment, "收盘后看见复盘提醒时")
  ...
  assert.equal(scene22.items[9].tradeMoment, "睡前想起还没复盘时")
...
test("official scene 23 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene23.items[0].tradeMoment, "突发消息弹出来时")
  assert.equal(scene23.items[0].os, "我次奥，真的假的。")
  assert.equal(scene23.items[0].hiddenThought, "机会来了。")
  assert.equal(scene23.items[0].reflection, "你还没验证消息，情绪已经开始交易。")
  assert.equal(scene23.items[9].tradeMoment, "收盘后担心消息落空时")
...
test("official scene 24 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene24.items[0].tradeMoment, "群里有人直接喊票时")
  ...
  assert.equal(scene24.items[9].tradeMoment, "关软件前想靠自己时")
...
test("official scene 25 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene25.items[0].tradeMoment, "热点板块集体大涨时")
  ...
  assert.equal(scene25.items[9].tradeMoment, "收盘后还盯着热点时")
...
test("official scene 26 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene26.items[0].tradeMoment, "看着它连续大跌时")
  ...
  assert.equal(scene26.items[9].tradeMoment, "收盘后盼明天反弹时")
...
test("official scene 27 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene27.items[0].tradeMoment, "看着它连拉几天时")
  ...
  assert.equal(scene27.items[9].tradeMoment, "收盘后还盼继续冲时")
...
test("official scene 28 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene28.items[0].tradeMoment, "深套后盯着成本线时")
  ...
  assert.equal(scene28.items[9].tradeMoment, "关软件前还念着回本时")
...
test("official scene 29 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene29.items[0].tradeMoment, "被套很久后看见反弹时")
  ...
  assert.equal(scene29.items[9].tradeMoment, "关软件前还念着解套时")
...
test("official scene 30 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene30.items[0].tradeMoment, "信号出现却没敢点时")
  ...
  assert.equal(scene30.items[9].tradeMoment, "收盘后盯着错过买点时")
...
test("official scene 31 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene31.items[0].tradeMoment, "刚成交就开始后悔时")
  ...
  assert.equal(scene31.items[9].tradeMoment, "收盘后还在担心时")
...
test("official scene 32 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene32.items[0].tradeMoment, "刚买入就想看账户时")
  ...
  assert.equal(scene32.items[9].tradeMoment, "收盘后统计打开次数时")
...
test("official scene 33 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene33.items[0].tradeMoment, "持仓横着不动时")
  ...
  assert.equal(scene33.items[9].tradeMoment, "关软件前又想换思路时")
...
test("official scene 34 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene34.items[0].tradeMoment, "刚刷到一条利好消息时")
  ...
  assert.equal(scene34.items[9].tradeMoment, "收盘后还等晚间消息时")
...
test("official scene 35 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene35.items[0].tradeMoment, "刚开始有浮盈时")
  ...
  assert.equal(scene35.items[9].tradeMoment, "收盘后庆幸自己卖了时")
...
test("official scene 36 content keeps the authored V3.1 wording", () => {
  ...
  assert.equal(scene36.items[9].tradeMoment, "收盘后把问题留到明天时")
