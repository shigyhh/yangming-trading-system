...
import { PanXinHeZhengReport } from "@/components/trade-review/PanXinHeZhengReport"
import { AssessmentShell, PrimaryButton } from "@/features/assessment/components"
import { syncTradeReviewBinding } from "@/features/data-binding/api-client"
...
type ReviewTimeframeKey = NonNullable<MultiTimeframeKlineContextResult["primaryTimeframe"]>
type ReviewTimeframeAvailability = MultiTimeframeKlineContextResult["availability"][ReviewTimeframeKey]

const timeframePanelMeta: Array<{ key: ReviewTimeframeKey; title: string; purpose: string }> = [
  { key: "101", title: "日线", purpose: "看大势" },
  { key: "60m", title: "60分钟", purpose: "看结构" },
  { key: "30m", title: "30分钟", purpose: "看下手那一刻" },
]

const timeframeAvailabilityLabels: Record<ReviewTimeframeAvailability, string> = {
  ok: "已读取",
  insufficient_data: "数据不足",
  missing: "未读取",
  error: "读取失败",
...
function buildMultiTimeframeNotice(result: MultiTimeframeKlineContextResult) {
  if (!result.primaryTimeframe) return "K线数据不足，已切换为手动盘证。"
  if (result.primaryTimeframe === "60m" && result.availability["30m"] !== "ok") {
    return "30分钟数据不足，已参考 60分钟。"
  }
  if (result.primaryTimeframe === "101" && result.availability["60m"] !== "ok") {
    return "60分钟数据不足，已参考日线。"
  }

  return "多周期盘面识别已完成，结果可继续手动校准。"
}

function getTimeframeCardValues(result: KlineContextResult | null) {
  return {
    ...
    dataSource: result?.dataSource || "insufficient_data",
  }
}

async function syncTradeReviewToServer(review: TradeReview) {
  const result = await syncTradeReviewBinding(review)
  const syncedAt = new Date().toISOString()

  updateTradeReview(review.id, {
    tradeReviewSyncStatus: result.ok ? "synced" : "failed",
    tradeReviewLastSyncedAt: syncedAt,
...
function ReviewPageContent() {
  ...
  const [multiTimeframeContext, setMultiTimeframeContext] = useState<MultiTimeframeKlineContextResult | null>(null)
  ...
  const [createdReview, setCreatedReview] = useState<TradeReview | null>(null)
  const [createdReportEvent, setCreatedReportEvent] = useState<OneThoughtEvent | null>(null)
  const [recentTradeReviews, setRecentTradeReviews] = useState<TradeReview[]>([])
  const [selectedHistoryReviewId, setSelectedHistoryReviewId] = useState("")

  useEffect(() => {
    const timer = window.setTimeout(() => {
      const pendingEvents = getPendingReviewEvents(DEFAULT_MIND_ARCHIVE_USER_ID)
      setEvents(pendingEvents)
      setRecentTradeReviews(listRecentTradeReviews(DEFAULT_MIND_ARCHIVE_USER_ID, 6))
  ...
  const historyReview = useMemo(
    () => recentTradeReviews.find((review) => review.id === selectedHistoryReviewId) ?? null,
    [recentTradeReviews, selectedHistoryReviewId],
  )
  ...
  const primaryKlineContext = multiTimeframeContext?.primaryTimeframe
    ? multiTimeframeContext.timeframes[multiTimeframeContext.primaryTimeframe]
    : null
  ...
  function markMarketContextEdited() {
    if (multiTimeframeContext) setMarketContextEdited(true)
  }
  ...
  async function handleAutoDetectKline() {
    ...
    const primaryResult = result.primaryTimeframe ? result.timeframes[result.primaryTimeframe] : null

    setMultiTimeframeContext(result)
    setMarketTrend(primaryResult?.marketTrend || "unclear")
    setPriceLocation(primaryResult?.priceLocation || "unclear")
    setPattern(primaryResult?.pattern || "unclear")
    setVolumeState(primaryResult?.volumeState || "unknown")
    setMarketContextEdited(false)
    setIsDetectingKline(false)
    setKlineNotice(buildMultiTimeframeNotice(result))
  }
  ...
  function handleSubmit() {
    ...
    const input: CreateTradeReviewInput = {
      ...
      marketContext: {
        ...
        confidence: primaryKlineContext?.confidence || multiTimeframeContext?.confidence,
        dataSource: primaryKlineContext?.dataSource || multiTimeframeContext?.dataSource || "manual",
        evidence: primaryKlineContext?.evidence,
        editedByUser: Boolean(multiTimeframeContext && marketContextEdited),
      },
      behaviorEvidence: {
        changedPlanIntraday,
        addedPosition,
    ...
    setSelectedHistoryReviewId(review.id)
  ...
  return (
    <AssessmentShell background="home-water" contentWidth="wide">
      ...
      <main className="review-page">
          ...
          <>
            <section className="review-panel" aria-label="上传盘证">
              ...
              {multiTimeframeContext ? (
                <div className="kline-result multi-timeframe-result" aria-label="多周期盘面识别结果">
                  <p>盘面识别结果</p>
                  <div className="timeframe-grid">
                    {timeframePanelMeta.map((item) => {
                      const result = multiTimeframeContext.timeframes[item.key]
                      const values = getTimeframeCardValues(result)
                      const availability = multiTimeframeContext.availability[item.key]
                      const isPrimary = multiTimeframeContext.primaryTimeframe === item.key

                      return (
                        <article className={isPrimary ? "timeframe-card is-primary" : "timeframe-card"} key={item.key}>
                          <div className="timeframe-card-head">
                            <div>
                              <h3>{item.title}</h3>
                              <span>{item.purpose}</span>
                            </div>
                            <strong>{isPrimary ? "主盘证" : timeframeAvailabilityLabels[availability]}</strong>
                          </div>
                          <dl>
                            <div>
                              <dt>趋势</dt>
                            ...
                            <div>
                              <dt>来源</dt>
                              <dd>{dataSourceLabels[values.dataSource]}</dd>
                            </div>
                          </dl>
                        </article>
                      )
                  ...
                  <div className="timeframe-summary" aria-label="最终盘证摘要">
                    {multiTimeframeContext.summary.dailyText ? <span>{multiTimeframeContext.summary.dailyText}</span> : null}
                    {multiTimeframeContext.summary.h60Text ? <span>{multiTimeframeContext.summary.h60Text}</span> : null}
                    {multiTimeframeContext.summary.m30Text ? <span>{multiTimeframeContext.summary.m30Text}</span> : null}
                    <strong>
                      {multiTimeframeContext.summary.finalText || "最终盘证：K线数据不足，已切换为手动盘证。"}
                    </strong>
                  </div>
                </div>
              ) : null}
            ...
            {createdReview ? <PanXinHeZhengReport review={createdReview} event={createdReportEvent} /> : null}
        ...
        <section className="review-panel review-history" aria-label="最近真实复盘">
          <p>最近真实复盘</p>
          <h2>已经写回的盘心合证。</h2>
          {recentTradeReviews.length ? (
            <div className="review-history-list">
              {recentTradeReviews.map((review) => (
                <article key={review.id} className={review.id === selectedHistoryReviewId ? "is-selected" : ""}>
                  <div>
                    <span>{formatPendingTime(review.createdAt)}</span>
                    <strong>{review.symbol}</strong>
                    <em>{heartJudgementLabels[review.heartJudgement]}</em>
                  </div>
                  <p>{review.reviewSummary?.practiceText || "报告待生成"}</p>
                  <button
                    type="button"
                    onClick={() => setSelectedHistoryReviewId((current) => (current === review.id ? "" : review.id))}
                  >
                    查看盘心合证报告
                  </button>
                </article>
            ...
            <span className="muted">暂无已保存的真实复盘。</span>
          )}
          {historyReview ? <PanXinHeZhengReport review={historyReview} event={null} /> : null}
        </section>
      </main>
      ...
      <style jsx>{`
        ...
        .timeframe-card-head span,
        .timeframe-card-head strong {
        ...
        .review-history-list article.is-selected {
          border-color: rgba(216, 183, 111, 0.34);
          background: rgba(216, 183, 111, 0.06);
        }

        .review-history-list article > div {
          align-items: center;
          display: flex;
          flex-wrap: wrap;
          gap: 12px;
