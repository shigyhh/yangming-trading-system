...
function ReviewPageContent() {
  ...
  async function handleAutoDetectKline() {
    ...
    const resolvedTimeframe = timeframe.trim() || "101"
    const resolvedEntryTime = entryTime.trim() || selectedEvent?.createdAt || ""

    if (!isKlineContextEnabled) {
      setKlineNotice("盘证自动识别暂未开启。你仍可手动选择趋势、位置、形态。")
      return
    }
    if (!resolvedSymbol) {
        ...
        {selectedEvent ? (
          <>
            <section className="review-panel" aria-label="上传盘证">
              ...
              {!isKlineContextEnabled ? (
                <div className="kline-notice">
                  盘证自动识别暂未开启。你仍可手动选择趋势、位置、形态。
                </div>
              ) : null}
