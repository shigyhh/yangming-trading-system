...
test("one-thought lake owns the resonance confirm button", () => {
  ...
  assert.match(zhaoxinSource, /此念照到你了吗？/)
  assert.match(zhaoxinSource, /不用判断对错，只记录此刻心的反应。/)
  assert.match(zhaoxinSource, /先选一个回响/)
  assert.match(zhaoxinSource, /选择此刻最真实的反应，再落印入今日所照。/)
  assert.match(zhaoxinSource, /长按落印入档/)
  assert.match(zhaoxinSource, /落印后，这一念会进入今日所照。/)
  assert.match(zhaoxinSource, /日后可在档案馆回看。/)
  assert.match(zhaoxinSource, /growth:\s*\{\s*title: "此念已落印",\s*sub: "这一念，已入今日所照。"/)
  assert.match(zhaoxinSource, /查看今日所照/)
  assert.match(zhaoxinSource, /再照一念/)
  assert.match(zhaoxinSource, /今日三念已满。/)
  assert.match(zhaoxinSource, /先回今日所照，看这几念怎么动。/)
  assert.match(zhaoxinSource, /照到了，先把这一念记下。/)
  assert.match(zhaoxinSource, /没照到也记下，后面继续校准。/)
  assert.match(zhaoxinSource, /愿意先停，就是今日功夫。/)
  assert.match(zhaoxinSource, /不必骗自己，心还在动也照实落下。/)
  assert.match(zhaoxinSource, /aria-label="长按落印入档"/)
  assert.match(zhaoxinSource, /disabled=\{!selectedReaction\}/)
  ...
  assert.doesNotMatch(zhaoxinSource, /先照见此念/)
  assert.doesNotMatch(zhaoxinSource, /长按落印，把今日心证收入心镜档案。/)
  ...
  assert.match(zhaoxinSource, /落印报告 · 照见第\{heartProofSequenceNumber\}念/)
  assert.match(zhaoxinSource, /growth-proof/)
  assert.match(zhaoxinSource, /growth-proof-ledger/)
  assert.match(zhaoxinSource, /今日所照落印记录/)
  assert.match(zhaoxinSource, /这一念，已入今日所照。/)
  ...
  assert.match(zhaoxinSource, /查看今日所照/)
  assert.doesNotMatch(zhaoxinSource, />继续照见一念</)
  assert.match(zhaoxinSource, /此念已落印/)
  assert.match(zhaoxinSource, /本机今日所照已保存。/)
  assert.match(zhaoxinSource, /要把这一念同步到档案馆吗？/)
  assert.match(zhaoxinSource, /同步到档案馆/)
  assert.match(zhaoxinSource, /只留在本机/)
  assert.match(zhaoxinSource, /留下手机号，换设备也能找回你的档案馆。/)
  assert.match(zhaoxinSource, /号码只用于找回与同步，不会公开。/)
  assert.match(zhaoxinSource, /不登录，也不影响本次落印。/)
  assert.match(zhaoxinSource, /已同步到档案馆。/)
  assert.match(zhaoxinSource, /换设备后，也能找回这一念。/)
  assert.match(zhaoxinSource, /已留在本机。/)
  assert.match(zhaoxinSource, /这一念仍会出现在今日所照。/)
  assert.match(zhaoxinSource, /查看档案馆/)
  assert.match(zhaoxinSource, /startPostSealContinue/)
  assert.match(zhaoxinSource, /changePostSealContinueThought/)
  assert.match(zhaoxinSource, /postSealContinueDraft/)
  assert.match(zhaoxinSource, /entry: "post_seal_continue"/)
  assert.match(zhaoxinSource, /previousEventId: sealedOneThoughtEventId/)
  assert.match(zhaoxinSource, /source: "one_thought_ritual"/)
  assert.match(zhaoxinSource, /ritualVersion: ONE_THOUGHT_RITUAL_VERSION/)
  assert.match(zhaoxinSource, /reflectionVersion: PRIVATE_REFLECTION_VERSION/)
  assert.match(zhaoxinSource, /stage: "zhaojian_this_thought"/)
  assert.match(zhaoxinSource, /drawTodayOneThought\(todayOneThought\.storedState/)
  assert.match(zhaoxinSource, /excludeThoughtIds: \[todayOneThought\.thoughtId\]/)
  assert.match(zhaoxinSource, /setStage\("thought"\)/)
  assert.match(zhaoxinSource, /onLakeModeChange\?\.\("thought"\)/)
  assert.match(zhaoxinSource, /archiveSyncPhase/)
  assert.match(zhaoxinSource, /beginArchiveSync/)
  assert.match(zhaoxinSource, /setArchiveSyncPhase\("form"\)/)
  assert.match(zhaoxinSource, /setArchiveSyncPhase\("synced"\)/)
  assert.match(zhaoxinSource, /setArchiveSyncPhase\("local"\)/)
  assert.match(zhaoxinSource, /growth-archive-coda/)
  assert.doesNotMatch(zhaoxinSource, /沉入长卷/)
  assert.doesNotMatch(zhaoxinSource, /去众念心湖看看/)
  ...
  assert.doesNotMatch(zhaoxinSource, /showArchiveLoginPrompt/)
  assert.doesNotMatch(zhaoxinSource, /requestArchiveLogin/)
  assert.doesNotMatch(zhaoxinSource, /archive-login-layer/)
  assert.doesNotMatch(zhaoxinSource, /aria-modal="true"/)
  assert.doesNotMatch(zhaoxinSource, /绑定手机号后，可跨设备查看心镜档案/)
  assert.doesNotMatch(zhaoxinSource, /心镜档案/)
  ...
  assert.match(zhaoxinSource, /看档案馆/)
  ...
  assert.match(zhaoxinSource, /换一念/)
  assert.match(zhaoxinSource, /is-thought-change/)
  assert.match(zhaoxinSource, /postSealContinueDraft && !shouldBlockTodayOneThoughtConfirmation/)
