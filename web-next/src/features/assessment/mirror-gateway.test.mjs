...
test("one-thought reveal cannot be frozen by stale hover pause state", () => {
  ...
  assert.match(zhaoxinSource, /setIsRevealPaused\(false\)[\s\S]*setStage\("mirrors"\)/)
  assert.match(zhaoxinSource, /setIsRevealPaused\(false\)[\s\S]*setStage\("thief"\)[\s\S]*setRevealStep\("thought"\)/)
})
