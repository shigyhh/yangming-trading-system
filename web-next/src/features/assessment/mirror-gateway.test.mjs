...
test("assessment keeps the locked mirror gateway and mounts the heart lake ritual inside it", () => {
  ...
  assert.match(assessmentPage, /<MirrorGateway/)
  assert.doesNotMatch(assessmentPage, /assessment-login/)
  assert.doesNotMatch(assessmentPage, /hasSavedPhone/)
  ...
  assert.match(source, /OneThoughtRitualFlow/)
  ...
  assert.ok(
    source.indexOf("<HeartLakeEngine") < source.indexOf("<OneThoughtRitualFlow"),
...
test("one-thought lake owns the resonance confirm button", () => {
  ...
  assert.match(zhaoxinSource, /oneThoughtReactionOptions/)
  assert.match(zhaoxinSource, /照见了/)
  assert.match(zhaoxinSource, /没照到/)
  assert.match(zhaoxinSource, /愿止一念/)
  assert.match(zhaoxinSource, /心还在动/)
  assert.match(zhaoxinSource, /落印入档/)
  assert.match(zhaoxinSource, /长按落印，把今日心证收入心镜档案。/)
  assert.match(zhaoxinSource, /aria-label="长按落印入档"/)
  ...
  assert.match(zhaoxinSource, /createOneThoughtRecord/)
  assert.match(zhaoxinSource, /saveOneThoughtRecord/)
  ...
  assert.match(zhaoxinSource, /今日照见已落印心证卷/)
  assert.match(zhaoxinSource, /今日照见已落印/)
  assert.match(zhaoxinSource, /本次一念/)
  assert.match(zhaoxinSource, /起念场景/)
  assert.match(zhaoxinSource, /照回/)
  assert.match(zhaoxinSource, /镜中显影/)
  assert.match(zhaoxinSource, /今日修行/)
  assert.match(zhaoxinSource, /growth-proof::before/)
  assert.match(zhaoxinSource, /沉入长卷/)
  assert.match(zhaoxinSource, /查看档案/)
  assert.match(zhaoxinSource, /handleArchiveNavigation/)
  ...
  assert.match(zhaoxinSource, /hasSavedPhone/)
  assert.match(zhaoxinSource, /requestArchiveLogin/)
  assert.match(zhaoxinSource, /showArchiveLoginPrompt/)
  assert.match(zhaoxinSource, /syncStatus: hasSavedPhone\(\) \? "synced" : "local_only"/)
  assert.match(zhaoxinSource, /归档前/)
  assert.match(zhaoxinSource, /把这枚心证，/)
  assert.match(zhaoxinSource, /存入你的档案。/)
  assert.match(zhaoxinSource, /绑定手机号后，可跨设备查看心镜档案、长卷与复测。手机号不会公开展示。/)
  assert.match(zhaoxinSource, /绑定并归档/)
  assert.match(zhaoxinSource, /本次仅本机保存/)
  assert.match(zhaoxinSource, /重新获取 \$/)
  assert.match(zhaoxinSource, /sendSmsCode\(archiveLoginPhone\)/)
  assert.match(zhaoxinSource, /verifySmsCode\(archiveLoginPhone, archiveLoginCode\)/)
  assert.match(zhaoxinSource, /setStorage\(assessmentStorageKeys\.userPhone, archiveLoginPhone\)/)
  assert.match(zhaoxinSource, /markCurrentInsightSyncStatus\("synced"\)/)
  assert.match(zhaoxinSource, /markCurrentInsightSyncStatus\("local_only"\)/)
  const liangzhiHoldBlock = zhaoxinSource.slice(
    zhaoxinSource.indexOf("const startLiangzhiHold"),
    zhaoxinSource.indexOf("const confirmThought"),
  )
  assert.doesNotMatch(liangzhiHoldBlock, /hasSavedPhone/)
  assert.doesNotMatch(liangzhiHoldBlock, /requestArchiveLogin/)
  assert.doesNotMatch(zhaoxinSource, /绑定并落印/)
  assert.doesNotMatch(zhaoxinSource, /稍后再说/)
  ...
  assert.doesNotMatch(zhaoxinSource, /我今天也有一念/)
  assert.doesNotMatch(zhaoxinSource, /thought-match-panel/)
  assert.doesNotMatch(zhaoxinSource, /matched-thought-result/)
