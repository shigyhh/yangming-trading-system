...
test("wechat h5 creation returns pay_url and uses real payer client ip", async () => {
  ...
  try {
    ...
    assert.equal(authorization.startsWith("WECHATPAY2-SHA256-RSA2048 "), true);
    assert.equal(authorization.startsWith("WECHATPAY2-SHA256-RSA2048,"), false);
    assert.match(authorization, /mchid="1900000001"/);
    assert.match(authorization, /nonce_str="[^"]+"/);
    assert.match(authorization, /signature="[^"]+"/);
    assert.match(authorization, /timestamp="[^"]+"/);
    assert.match(authorization, /serial_no="TESTMERCHANTSERIAL"/);
  } finally {
    await rm(tempDir, { recursive: true, force: true });
    restoreEnv();
    globalThis.fetch = originalFetch;
...
test("wechat request signing string keeps five-line format", async () => {
  const source = await readFile(new URL("../src/services/payments/wechatPay.js", import.meta.url), "utf8");
  assert.equal(source.includes("signer.update(`${method}\\n${path}\\n${timestamp}\\n${nonce}\\n${bodyText}\\n`);"), true);
});
...
test("wechat provider error code is returned and debug log is redacted", async () => {
  ...
  try {
    ...
    assert.doesNotMatch(joined, /signature/i);
